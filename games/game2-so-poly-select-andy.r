...
server <- function( input, output, session ){
  ...
  foundational.map <- shiny::reactive({
    ...
    #andy
    mv <- mapview::mapview(sfafricountries, zcol='name', addLegend=FALSE)
    mv@map
  ...
  shiny::observeEvent( input$myMap_shape_click, {
    ...
    #andy - how to choose which column of sfafricountries to subset by ? trying first
    lines.of.interest <- sfafricountries[ which( sfafricountries$name %in% click.list$ids ) , ]    
    ...
    #andy remove @data because sf not sp  
      
    } else if( !click$id %in% lines.of.interest$id ){
